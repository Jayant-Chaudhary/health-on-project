...
IMAGE_SUFFIXES = (".jpg", ".png", ".jpeg", ".bmp", ".tiff", ".tif")


class _LazyOCR:
    """Builds the PaddleOCR predictor on first use, not at import.

    Loading at import made every worker pay for these models even when only
    /document/process (which has its own predictor) is used. The lock
    serializes inference: a predictor must not run on two threads at once.
    """
    ...
    def predict(self, *args, **kwargs):
        with self._lock:
            ...
            # Materialised inside the lock: predict() returns a generator.
            return list(self._ocr.predict(*args, **kwargs))
...
def extract_ocr_data(result):
    ...
    def _extract_from_dict(d: dict):
        ...
        rec_scores = core.get('rec_scores')
        rec_scores = [float(x) for x in _np_to_list(rec_scores)] if rec_scores is not None else []
        return {
...
def predict_by_file(file: UploadFile):
    # Sync on purpose: FastAPI runs it on the thread pool, so a long OCR call
    # does not block the event loop (and every other request) while it runs.
    restfulModel: RestfulModel = RestfulModel()
    if (file.filename or "").lower().endswith(IMAGE_SUFFIXES):  # 支持更多图片格式
        ...
        raise HTTPException(
            ...
            detail="Unsupported image format; expected .jpg, .jpeg, .png, .bmp, .tif or .tiff"
        )
    return restfulModel


@router.get('/predict-by-url', response_model=RestfulModel, summary="识别图片 URL")
def predict_by_url(imageUrl: str):
    # 直接使用URL进行predict
